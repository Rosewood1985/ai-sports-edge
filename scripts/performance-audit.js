...
fs.writeFileSync(reportPath, `AI Sports Edge Performance Audit\n${'-'.repeat(40)}\nDate: ${new Date().toLocaleString()}\n\n`);
...
async function runAudit() {
  ...
  try {
    ...
    for (const url of URLS) {
      ...
      // Take screenshot
      const screenshotPath = path.join(SCREENSHOTS_DIR, `${url.split('/').pop() || 'home'}-${timestamp}.png`);
      ...
      const fcpMetric = await page.evaluate(() => {
        return new Promise(resolve => {
          ...
          setTimeout(() => resolve(performance.timing.domContentLoadedEventEnd - performance.timing.navigationStart), 5000);
        });
      ...
      const seoChecks = [
        ...
        { name: 'Has meta description', check: async () => await page.$eval('meta[name="description"]', el => el.content.length > 0).catch(() => false) },
        { name: 'Has canonical link', check: async () => await page.$('link[rel="canonical"]').then(el => !!el).catch(() => false) },
        { name: 'Has Open Graph tags', check: async () => await page.$('meta[property^="og:"]').then(el => !!el).catch(() => false) },
        { name: 'Has structured data', check: async () => await page.$('script[type="application/ld+json"]').then(el => !!el).catch(() => false) },
        { name: 'Has h1 heading', check: async () => await page.$('h1').then(el => !!el).catch(() => false) },
        { name: 'Images have alt text', check: async () => {
          const images = await page.$$('img');
          if (images.length === 0) return true;
          const imagesWithAlt = await page.$$eval('img', imgs => imgs.filter(img => img.alt).length);
      ...
      // Check accessibility
      const a11yChecks = [
        { name: 'Has lang attribute', check: async () => await page.$eval('html', el => !!el.lang).catch(() => false) },
        { name: 'Form inputs have labels', check: async () => {
        ...
        { name: 'Sufficient color contrast', check: async () => {
          // This is a simplified check - a real check would use color contrast algorithms
          return true; // Assume passed for now
        }},
        { name: 'Has skip link', check: async () => await page.$('a[href="#main"], a[href="#content"]').then(el => !!el).catch(() => false) },
        { name: 'Interactive elements are keyboard accessible', check: async () => {
      ...
      appendToReport(`Performance Score: ${getScoreEmoji(performanceScore)} ${performanceScore}/100`);
