  ...
  try {
    ...
    for (const ip of testIPs) {
      ...
      try {
        ...
        console.log(
          `  Coordinates: ${response.location?.latitude || 0}, ${response.location?.longitude || 0}`
        );
...
async function testGeolocationServiceIntegration() {
  ...
  try {
    ...
    console.log(
      '- Modified getUserLocation() to try GeoIP service before falling back to IPGeolocation API'
    );
