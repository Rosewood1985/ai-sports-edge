...
function main() {
  ...
  if (!fileExists(envPath)) {
    ...
    console.log(
      `${colors.yellow}Please create a ${ENV_FILE} file by copying ${ENV_EXAMPLE_FILE}:${colors.reset}`
    );
  ...
  const placeholderVars = REQUIRED_VARIABLES.filter(key => {
    const value = env[key];
    return (
      value && (value.includes('your-') || value.includes('placeholder') || value === 'REPLACE_ME')
    );
  });

  if (placeholderVars.length > 0) {
    console.warn(
      `${colors.yellow}Warning: The following variables appear to have placeholder values:${colors.reset}`
    );
