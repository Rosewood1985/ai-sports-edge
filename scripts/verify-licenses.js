...
async function main() {
  try {
    ...
    console.log(`- Compatible Licenses: ${results.filter(r => r.compatibility === LicenseCompatibility.Compatible).length}`);
...
function getLicenseInfo(packageName) {
  try {
    ...
    const npmInfo = JSON.parse(
      execSync(`npm view ${packageName} --json`, { encoding: 'utf8' })
    );
