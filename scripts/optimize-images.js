...
const optimizeImage = async (filePath) => {
  ...
  if (fileExt === '.jpg' || fileExt === '.jpeg') {
    await image
      .jpeg({ quality, progressive: true, optimizeCoding: true })
...
const main = async () => {
  ...
  try {
    ...
    console.log(`Size reduction: ${chalk.green(formatBytes(totalSavings))} (${chalk.green(totalSavingsPercentage.toFixed(2))}%)`);
