...
async function testCommandInjectionPrevention(results) {
  for (const test of SECURITY_TESTS.commandInjection) {
    ...
    try {
      ...
      recordTestResult(results, test, 'FAILED', `Could not import secure service: ${importError.message}`);
    }
  }
}
...
async function testServiceIntegration(results) {
  ...
  for (const test of integrationTests) {
    ...
    try {
      const serviceName = test.name.toLowerCase()
        .replace('secure ', '')
        .replace(' available', '')
        .replace(/ /g, '');
...
async function checkForVulnerableServices() {
  ...
  for (const file of vulnerableFiles) {
    ...
    try {
      ...
      if (content.includes('exec(') || content.includes('spawn(') && !content.includes('secureCommandService')) {
