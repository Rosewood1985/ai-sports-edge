...
export class RacingPerformanceNormalizer implements PerformanceNormalizer {
  ...
  analyzeTrends(historicalPerformance: NormalizedPerformance[]): PerformanceTrend {
    ...
    const sortedPerformance = historicalPerformance.sort(
      (a, b) => new Date(a.normalizedAt).getTime() - new Date(b.normalizedAt).getTime()
    );
  ...
  private normalizeNascarPerformance(driver: StandardizedNascarDriver): NormalizedPerformance {
    ...
    return {
      ...
      sportSpecific: {
        ...
        avgFinishNormalized: Math.max(
          0,
          1 - driver.stats.averageFinish / NASCAR_AVERAGE_FIELD_SIZE
        ),
  ...
  private normalizeHorseRacingPerformance(horse: StandardizedHorse): NormalizedPerformance {
    ...
    return {
      ...
      sportSpecific: {
        ...
        earningsPerStartNormalized: Math.min(
          horse.stats.earningsPerStart / HORSE_RACING_MAX_EARNINGS_PER_START,
          1
        ),
  ...
  private calculateNascarRecentFormScore(recentForm: any): number {
    ...
    const avgPosition =
      recentForm.positions.reduce((sum, pos) => sum + pos, 0) / recentForm.positions.length;
  ...
  private estimateHorseCompetitionQuality(horse: StandardizedHorse): number {
    ...
    const competitionIndicator =
      (horse.stats.winRate / 100) * (horse.stats.earningsPerStart / 10000);
  ...
  private calculateHorseVersatility(preferences: any): number {
    ...
    const distanceRange =
      (preferences.preferredDistance.max - preferences.preferredDistance.min) / 10;
  ...
  private calculatePeerGroupStats(peerGroup: NormalizedPerformance[]) {
    ...
    coreMetrics.forEach(metric => {
      avgCore[metric] =
        peerGroup.reduce((sum, peer) => sum + peer.core[metric as keyof typeof peer.core], 0) /
        peerGroup.length;
    });
    ...
    const topPerformer = peerGroup.reduce((best, current) => {
      const bestScore =
        (best.core.winRate +
          best.core.successRate +
          best.core.consistency +
          best.core.improvement) /
        4;
      const currentScore =
        (current.core.winRate +
          current.core.successRate +
          current.core.consistency +
          current.core.improvement) /
        4;
  ...
  private calculateRankings(
    ...
    const overallScore =
      (performance.core.winRate +
        performance.core.successRate +
        performance.core.consistency +
        performance.core.improvement) /
      4;

    const peerScores = peerGroup.map(
      peer =>
        (peer.core.winRate +
          peer.core.successRate +
          peer.core.consistency +
          peer.core.improvement) /
        4
    );
    ...
    const variance =
      peerScores.reduce((sum, score) => sum + Math.pow(score - mean, 2), 0) / peerScores.length;
  ...
  private calculatePerformanceGaps(performance: NormalizedPerformance, peerStats: any) {
    const performanceScore =
      ...
      4;
    const averageScore =
      (peerStats.average.core.winRate +
        peerStats.average.core.successRate +
        peerStats.average.core.consistency +
        peerStats.average.core.improvement) /
      4;
    const topScore =
      (peerStats.top.core.winRate +
        peerStats.top.core.successRate +
        peerStats.top.core.consistency +
        peerStats.top.core.improvement) /
      4;
  ...
  private calculateOverallTrend(performances: NormalizedPerformance[]) {
    ...
    const scores = performances.map(
      p => (p.core.winRate + p.core.successRate + p.core.consistency + p.core.improvement) / 4
    );
  ...
  private generateForecast(performances: NormalizedPerformance[]) {
    ...
    const recentScore =
      (recentPerformance.core.winRate +
        recentPerformance.core.successRate +
        recentPerformance.core.consistency +
        recentPerformance.core.improvement) /
      4;
