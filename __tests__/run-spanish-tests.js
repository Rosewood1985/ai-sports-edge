...
try {
  ...
  const testCommand = 'npx jest --testMatch="**/__tests__/i18n/spanish-language-test.js" --json --outputFile=test-results/spanish-test-results.json';
  execSync(testCommand, { stdio: 'inherit' });
  
  // Read the test results
  const testResults = JSON.parse(fs.readFileSync(path.join(testResultsDir, 'spanish-test-results.json'), 'utf8'));
