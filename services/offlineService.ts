...
class OfflineService {
   ...
   * @returns Document data or null if not found
   */
  private async getDataFromFirestore<T>(collectionPath: string, docId: string): Promise<T | null> {
    try {
   ...
   */
  private async cacheData<T>(cacheKey: string, data: T, cacheTTL: number): Promise<void> {
    try {
      const now = Date.now();
   ...
   * @returns Document ID
   */
  public async saveData(collectionPath: string, data: any, docId?: string): Promise<string> {
    try {
   ...
   * @param docId Document ID
   */
  public async deleteData(collectionPath: string, docId: string): Promise<void> {
    try {
   ...
   * @param docId Document ID
   */
  private async deleteDataFromFirestore(collectionPath: string, docId: string): Promise<void> {
    try {
      const docRef = doc(firestore, collectionPath, docId);
            ...
            case 'update':
              if (operation.collection && operation.data) {
                ...
                if (
                  ...
                  operation.docId &&
                  operation.docId.startsWith('temp_')
                ) {
