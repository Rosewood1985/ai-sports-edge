...
class NcaaOddsService {
  ...
  async getGameOdds(gameId, gender = 'mens') {
    try {
      const allOdds = gender === 'mens' 
        ? await this.getMensMoneylineOdds() 
        : await this.getWomensMoneylineOdds();
  ...
  async getMarchMadnessOdds(gender = 'mens') {
    try {
      ...
      const events = gender === 'mens' 
        ? await this.getUpcomingMensEvents() 
        : await this.getUpcomingWomensEvents();
      
      // Filter for March Madness tournament
      const marchMadnessEvents = events.filter(event => 
        event.tournament && event.tournament.toLowerCase().includes('ncaa tournament')
      );
