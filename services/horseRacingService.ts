...
class HorseRacingService {
  ...
  async clearCache(): Promise<void> {
    try {
      ...
      const cacheKeys = keys.filter(
        key => key.startsWith(RACES_CACHE_KEY) || key.startsWith(TRACKS_CACHE_KEY)
      );
  ...
  private async fetchRaceEntries(count: number, raceId: string): Promise<Horse[]> {
    ...
    if (
      !process.env.HORSE_RACING_API_KEY &&
      !process.env.SPORTS_DATA_API_KEY &&
      !process.env.RPSCRAPE_ENABLED
    ) {
      console.warn(
        'Horse racing data sources not configured. Please set RPSCRAPE_ENABLED=true, HORSE_RACING_API_KEY, or SPORTS_DATA_API_KEY.'
      );
    }
    ...
    for (let i = 0; i < horses.length; i++) {
      for (let j = 0; j < horses.length; j++) {
        if (i !== j) {
          ...
          const combinedOdds =
            (horse1.currentOdds || horse1.odds || 5) *
        ...
        for (let k = 0; k < horses.length; k++) {
          if (i !== j && i !== k && j !== k) {
            ...
            const combinedOdds =
              (horse1.currentOdds || horse1.odds || 5) *
              (horse2.currentOdds || horse2.odds || 5) *
              (horse3.currentOdds || horse3.odds || 5) *
              0.7;
  ...
  private generateMockPrediction(race: Race): RacePrediction {
    ...
    if (predictions.length > 2) {
      ...
      suggestedBets.push({
        ...
        recommendedAmount: 2,
        expectedValue:
          topHorse.expectedValue * secondHorse.expectedValue * thirdHorse.expectedValue,
        confidenceScore:
          (topHorse.confidenceScore + secondHorse.confidenceScore + thirdHorse.confidenceScore) / 3,
    ...
    const trackBias = {
      ...
      ] as 'early_speed' | 'stalker' | 'closer' | 'none',
      favoredPostPositions: [] as number[],
      biasStrength: ['strong', 'moderate', 'weak', 'none'][Math.floor(Math.random() * 4)] as
        | 'strong'
        | 'moderate'
        | 'weak'
        | 'none',
    ...
    const paceScenario = {
      projectedPace: ['fast', 'moderate', 'slow'][Math.floor(Math.random() * 3)] as
        | 'fast'
        | 'moderate'
        | 'slow',
    ...
    if (paceScenario.projectedPace === 'fast' && paceScenario.closers.length > 0) {
      const randomCloser =
        paceScenario.closers[Math.floor(Math.random() * paceScenario.closers.length)];
      paceScenario.paceAdvantage = randomCloser;
    } else if (paceScenario.projectedPace === 'slow' && paceScenario.earlyPaceSetters.length > 0) {
      const randomPaceSetter =
        paceScenario.earlyPaceSetters[
          Math.floor(Math.random() * paceScenario.earlyPaceSetters.length)
        ];
