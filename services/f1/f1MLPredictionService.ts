...
export class F1MLPredictionService {
  ...
  private async runPredictionModels(features: F1PredictionFeatures, historicalData: any[]): Promise<any> {
    const models = [
  ...
  private async predictConstructorWinner(features: F1PredictionFeatures, constructors: F1Constructor[]) {
    const predictions = [];
  ...
  private async predictDNFs(features: F1PredictionFeatures, drivers: F1Driver[]) {
    ...
    for (const driver of drivers) {
      ...
      if (dnfProbability > 0.05) { // Only include if >5% chance
        predictions.push({
          driverId: driver.driverId,
  ...
  private async getConstructorFeatures(constructor: F1Constructor, baseFeatures: F1PredictionFeatures) {
    const stats = constructor.seasonStats;
    if (!stats) return baseFeatures;

    return {
      ...baseFeatures,
      constructorChampionshipPosition: await this.getConstructorChampionshipPosition(constructor.constructorId),
  ...
  private calculateWinProbability(features: F1PredictionFeatures): number {
    ...
    if (features.circuitType === 1) { // Street circuit
  ...
  private async getHistoricalData(circuitId: string): Promise<any[]> {
    const snapshot = await this.db.collection('f1_historical_data')
  ...
  private async getDriverChampionshipPosition(driverId: string): Promise<number> {
    const snapshot = await this.db.collection('f1_driver_standings')
  ...
  private async getConstructorChampionshipPosition(constructorId: string): Promise<number> {
    const snapshot = await this.db.collection('f1_constructor_standings')
  ...
  private async getCircuitExperience(driverId: string, features: F1PredictionFeatures): Promise<number> {
  ...
  private async storePrediction(prediction: F1Prediction): Promise<void> {
    await this.db.collection('f1_predictions').doc(prediction.raceId).set({
      ...prediction,
