...
class SecureImagePreprocessingService {
  ...
  async validateImageFormat(imagePath) {
    try {
      ...
      if (w < 10 || h < 10) {
        throw new SecurityError(
          `Image dimensions too small: ${w}x${h}`,
  ...
  async generateSecureOutputPath(inputPath, suffix) {
    ...
    if (!resolvedPath.startsWith(path.resolve(this.tempDirectory))) {
      throw new SecurityError(
        'Generated path outside allowed directory',
        'INVALID_OUTPUT_PATH'
      );
    }
  ...
  async validateOutputFile(outputPath) {
    try {
      ...
      try {
        ...
        console.warn('[SECURE PREPROCESSING] Could not clean up invalid output:', unlinkError.message);
      }
    ...
    for (const filePath of processedFiles) {
      if (filePath !== originalPath) {
        try {
          ...
          console.warn('[SECURE PREPROCESSING] Warning: Could not clean up processed file:', error.message);
        }
      }
    }
  }
