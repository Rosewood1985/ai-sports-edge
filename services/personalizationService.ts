...
class PersonalizationService {
  ...
  public async initialize(): Promise<void> {
    ...
    try {
      ...
      if (Object.keys(this.userPreferences).length === 0) {
        ...
        const isSpanishUser =
          deviceLang.startsWith('es') || deviceLang === 'spa' || deviceLang.includes('spanish');
  ...
  public async setDisplayPreferences(
  ...
  private async saveUserPreferences(): Promise<void> {
    ...
    while (retries < maxRetries) {
      try {
        ...
        await AsyncStorage.setItem(storageKey, JSON.stringify(preferencesToSave));
        ...
        if (retries >= maxRetries) {
          ...
          analyticsService
            .trackEvent(AnalyticsEventType.CUSTOM, {
