...
export const getUserPreferences = async (userId) => {
  try {
    const userDoc = await getDoc(firebaseService.firestore.firebaseService.firestore.doc(db, 'users', userId));
...
export const updateUserPreferences = async (userId, preferences) => {
  try {
    ...
    if (!userDoc.exists()) {
      ...
      await setDoc(firebaseService.firestore.firebaseService.firestore.doc(db, 'users', userId), {
        preferences,
        createdAt: new Date()
      });
    } else {
      // User exists, update preferences
      await updateDoc(firebaseService.firestore.firebaseService.firestore.doc(db, 'users', userId), {
        preferences
      });
    }
  } catch (error) {
...
export const updatePreference = async (userId, path, value) => {
  try {
    ...
    if (!userDoc.exists()) {
      ...
      await updateDoc(firebaseService.firestore.firebaseService.firestore.doc(db, 'users', userId), updateData);
    }
  } catch (error) {
