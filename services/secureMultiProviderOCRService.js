...
class SecureMultiProviderOCRService {
  ...
  async processWithProvider(imagePath, provider, options = {}) {
    ...
    if (!providerConfig || !providerConfig.available) {
      throw new SecurityError(`Provider not available: ${provider}`, 'PROVIDER_NOT_AVAILABLE');
    }
    ...
    switch (provider) {
      ...
      default:
        throw new SecurityError(`Unsupported provider: ${provider}`, 'UNSUPPORTED_PROVIDER');
    }
  }
  ...
  async processTesseractSecure(imagePath, options = {}) {
    const { language = 'eng', oem = '3', psm = '6', configOptions = {} } = options;
    ...
    try {
      ...
      const args = [imagePath, 'stdout', '-l', validLanguage, '--oem', oem, '--psm', psm];
      ...
      console.log(
        `[SECURE TESSERACT] Processing: ${path.basename(imagePath)} with language: ${validLanguage}`
      );

      // Execute secure Tesseract command
      const result = await secureCommandService.executeSecureCommand('tesseract', args, {
