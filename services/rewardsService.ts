...
class RewardsService {
  ...
  async recordBetPlaced(userId: string): Promise<{
    ...
    try {
      ...
      if (
        loyaltyMasterAchievement &&
        !loyaltyMasterAchievement.isUnlocked &&
        rewards.loyaltyLevel === 'Platinum'
      ) {
  ...
  async recordReferral(
    ...
    try {
      ...
      const socialButterflyAchievement = rewards.achievements.find(
        a => a.id === 'social_butterfly'
      );
      if (
        socialButterflyAchievement &&
        !socialButterflyAchievement.isUnlocked &&
        rewards.referralCount >= 5
      ) {
  ...
  async getReferralLeaderboard(
    ...
    try {
      ...
      const mockLeaderboard = [
        {
          userId: 'user1',
          displayName: 'BettingPro',
          referralCount: 24,
          badgeType: 'hall-of-fame' as BadgeType,
        },
        {
          userId: 'user2',
          displayName: 'SportsFan99',
          referralCount: 18,
          badgeType: 'elite' as BadgeType,
        },
        {
          userId: 'user3',
        ...
        {
          userId: 'user5',
          displayName: 'OddsWizard',
          referralCount: 10,
          badgeType: 'elite' as BadgeType,
        },
        {
          userId: 'user6',
          displayName: 'StatsGuru',
          referralCount: 8,
          badgeType: 'rookie' as BadgeType,
        },
        {
          userId: 'user7',
          displayName: 'PicksExpert',
          referralCount: 7,
          badgeType: 'rookie' as BadgeType,
        },
        {
          userId: 'user8',
          displayName: 'BetInsider',
          referralCount: 6,
          badgeType: 'rookie' as BadgeType,
        },
        {
   ...
   */
  async updateLeaderboardPrivacy(userId: string, privacy: LeaderboardPrivacy): Promise<boolean> {
    try {
      let rewards = await this.getUserRewards(userId);
      if (!rewards) {
        rewards = await this.initializeUserRewards(userId);
