...
export class SoccerMLPredictionService {
  ...
  private async extractFeatures(match: SoccerMatch): Promise<MLFeatures> {
    ...
    const features: MLFeatures = {
      ...
      homeShootingAccuracy:
        (homeAdvancedStats.shotsOnTargetPerGame || 4) / (homeAdvancedStats.shotsPerGame || 12),
      awayShootingAccuracy:
        (awayAdvancedStats.shotsOnTargetPerGame || 4) / (awayAdvancedStats.shotsPerGame || 12),
      ...
      h2hHomeWinRate:
        headToHead.totalMeetings > 0 ? headToHead.homeWins / headToHead.totalMeetings : 0.33,
      h2hAwayWinRate:
        headToHead.totalMeetings > 0 ? headToHead.awayWins / headToHead.totalMeetings : 0.33,
      h2hDrawRate:
        headToHead.totalMeetings > 0 ? headToHead.draws / headToHead.totalMeetings : 0.33,
      ...
      pointsPerGameDiff:
        homeTeamStats.points / homeTeamStats.matchesPlayed -
      ...
      squadValueDifference:
        (homeAdvancedStats.squadValue || 50000000) - (awayAdvancedStats.squadValue || 50000000),
  ...
  private async getHeadToHeadFeatures(
    homeTeamId: string,
    awayTeamId: string,
    competition: string
  ): Promise<any> {
  ...
  private predictMatchResult(features: MLFeatures): {
    homeWin: number;
    draw: number;
    awayWin: number;
  } {
    ...
    return {
      ...
      draw: Math.max(0.1, Math.min(0.5, drawProb / total)),
  ...
  private calculateExactScoreProbabilities(
    homeXG: number,
    awayXG: number
  ...
  private calculateAsianHandicap(resultPrediction: any, goalsPrediction: any): any[] {
  ...
  private calculateHandicapProbability(
    homeXG: number,
    awayXG: number,
    handicap: number,
    side: 'home' | 'away'
  ...
  private calculateFeatureImportance(
    ...
    const importanceMap = [
      ...
      { feature: 'homeMomentum', importance: 0.1 },
      { feature: 'awayMomentum', importance: 0.1 },
  ...
  private calculateModelConfidence(features: MLFeatures, resultPrediction: any): number {
    ...
    const maxProb = Math.max(
      resultPrediction.homeWin,
      resultPrediction.draw,
      resultPrediction.awayWin
    );
  ...
  private async storePrediction(matchId: string, prediction: MLPrediction): Promise<void> {
    await this.db
      .collection('soccer_ml_predictions')
      .doc(matchId)
      .set({
        ...prediction,
