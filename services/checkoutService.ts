...
export class CheckoutService {
  private createCheckoutSessionCallable = httpsCallable<CheckoutSessionData, CheckoutSessionResponse>(
    functions, 
    'createCheckoutSession'
  );
  
  private handleSuccessfulPaymentCallable = httpsCallable<{sessionId: string}, {success: boolean}>(
    functions, 
   ...
   */
  async createCheckoutSession(data: Omit<CheckoutSessionData, 'userId'>): Promise<CheckoutSessionResponse> {
    try {
   ...
   */
  async createCheckoutSessionWithEduDiscount(data: Omit<CheckoutSessionData, 'userId'>): Promise<CheckoutSessionResponse> {
    try {
      const user = auth.currentUser;
      if (!user?.email) {
