...
import { analyticsService, AnalyticsEventType, AnalyticsProperties } from './analyticsService';
...
class ABTestingService {
  ...
  public async getExperimentResults(experimentId: string): Promise<ExperimentResult[]> {
    ...
    for (const result of results) {
      result.conversionRate =
        result.impressions > 0 ? (result.conversions / result.impressions) * 100 : 0;
      result.averageConversionValue =
        result.conversions > 0 ? result.averageConversionValue / result.conversions : 0;
    }
  ...
  private async saveExperiments(): Promise<void> {
    try {
      ...
      await AsyncStorage.setItem(this.STORAGE_KEYS.EXPERIMENTS, JSON.stringify(experiments));
    } catch (error) {
