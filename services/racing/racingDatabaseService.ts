...
export class RacingDatabaseService {
  ...
  async storeNascarRace(
    ...
    try {
      ...
      if (mlFeatures.length > 0) {
        await this.cacheService.cacheMLFeatures(
          raceData.id,
          'nascar',
  ...
  async storeNascarDriver(driverData: StandardizedNascarDriver): Promise<string> {
    try {
      ...
      const existingDriver = await this.findOne('nascarDrivers', {
   ...
   */
  async getNascarRaces(
    options: QueryOptions = {}
  ): Promise<DatabaseQueryResult<NascarRaceDocument>> {
   ...
   */
  async getDriverTrackPerformance(driverId: string, trackId: string): Promise<any> {
   ...
   */
  async bulkUpdateRaceData(sport: RacingSport, updates: any[]): Promise<BulkOperationResult> {
      ...
      for (const update of updates) {
        try {
          if (update.operation === 'insert') {
            ...
            await this.updateDocument(this.getCollectionName(sport), update.filter, update.data);
            result.updated++;
  ...
  private calculateRacePriority(raceData: any): number {
    ...
    const daysFromNow = Math.abs(
      (new Date().getTime() - new Date(raceData.raceDate).getTime()) / (1000 * 60 * 60 * 24)
    );
  ...
  private calculateFeatureCompleteness(features: MLFeatureVector): number {
    ...
    const completeFeatures = Object.values(features.features).filter(category => {
