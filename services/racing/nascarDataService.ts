...
class NascarDataService {
  ...
  async getSeasonData(
    season: number,
    series: 'Cup' | 'Xfinity' | 'Truck' = 'Cup'
  ): Promise<NascarSeasonData> {
    ...
    try {
      ...
      throw new Error(
        `Failed to fetch NASCAR season data: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  private async fetchDriverStats(season: number, series: string): Promise<NascarDriverStats[]> {
    ...
    try {
      ...
      return this.calculateDriverStatsFromRaces(
        races,
        season,
        series as 'Cup' | 'Xfinity' | 'Truck'
      );
    } catch (error) {
   ...
   */
  private async fetchDriverStatsForSeason(
    driverId: string,
    season: number,
    series: string
  ): Promise<NascarDriverStats | null> {
    try {
      const driverStats = await this.fetchDriverStats(season, series);
   ...
   */
  private async fetchTrackRaces(
    trackName: string,
    seasons: number[],
    series: string
  ): Promise<NascarRaceData[]> {
    try {
   ...
   */
  private calculateDriverStatsFromRaces(
    races: NascarRaceData[],
    season: number,
    series: 'Cup' | 'Xfinity' | 'Truck'
  ): NascarDriverStats[] {
  ...
  private determineTrackType(
    trackName: string
  ): 'superspeedway' | 'intermediate' | 'short' | 'road_course' | 'dirt' {
    ...
    if (track.includes('daytona') || track.includes('talladega')) {
      ...
      track.includes('road') ||
      track.includes('course') ||
      track.includes('glen') ||
      track.includes('roval')
    ) {
      return 'road_course';
    } else if (
