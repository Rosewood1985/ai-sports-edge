...
class HorseRacingDataService {
   ...
   */
  async getRaceWithMLFeatures(course: string, date: string, raceTime: string): Promise<{ race: Race; mlFeatures: HorseRacingMLFeatures[] } | null> {
    const cacheKey = `${this.CACHE_PREFIX}_ml_features_${course}_${date}_${raceTime}`;
    
    try {
      // Check cache first
      const cachedData = await cacheService.get<{ race: Race; mlFeatures: HorseRacingMLFeatures[] }>(cacheKey);
      if (cachedData) {
        return cachedData;
      }
   ...
   */
  private async convertRpscrapeToRace(rpscrapeRace: RpscrapeRace, meeting: RpscrapeMeeting, date: string): Promise<Race> {
    const track: Track = {
   ...
   */
  private async generateMLFeatures(race: Race, rpscrapeRace: RpscrapeRace): Promise<HorseRacingMLFeatures[]> {
