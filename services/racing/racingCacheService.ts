...
export class RacingCacheService {
  ...
  private async evictFromColdCache(requiredSpace: number): Promise<void> {
    const entries = Array.from(this.coldCache.entries())
      .sort((a, b) => a[1].metadata.lastAccessed.getTime() - b[1].metadata.lastAccessed.getTime());
  ...
  private getTierSize(tier: string): number {
    switch (tier) {
      case 'hot': return this.hotCache.size;
      case 'warm': return this.warmCache.size;
      case 'cold': return this.coldCache.size;
      default: return 0;
    }
  }
