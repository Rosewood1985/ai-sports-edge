...
export const getVenueWeather = async (
  ...
  try {
    ...
    const venueDoc = await getDoc(firebaseService.firestore.firebaseService.firestore.doc(db, 'venues', venueId));
    if (!venueDoc.exists()) {
...
export const getGameWeather = async (
  ...
  try {
    ...
    const gameDoc = await getDoc(firebaseService.firestore.firebaseService.firestore.doc(db, 'games', gameId));
    if (!gameDoc.exists()) {
...
export const getWeatherPerformanceInsights = async (
  ...
  try {
    ...
    // Get player details
    const playerDoc = await getDoc(firebaseService.firestore.firebaseService.firestore.doc(db, 'players', playerId));
    if (!playerDoc.exists()) {
      throw new Error(`Player not found: ${playerId}`);
    }
    
    // Check if we have weather correlations in Firestore
    const correlationsDoc = await getDoc(firebaseService.firestore.firebaseService.firestore.doc(db, 'weatherCorrelations', playerId));
    ...
    if (db) {
      await setDoc(firebaseService.firestore.firebaseService.firestore.doc(db, 'weatherCorrelations', playerId), {
