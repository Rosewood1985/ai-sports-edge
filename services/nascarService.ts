...
export const getUpcomingRaces = async (): Promise<NascarRace[]> => {
  const transaction = sentryService.startTransaction(
    'nascar-get-upcoming-races',
    'query',
    'Fetch upcoming NASCAR races'
  );
...
export const getDriverStandings = async (): Promise<NascarDriver[]> => {
  const transaction = sentryService.startTransaction(
    'nascar-get-driver-standings',
    'query',
    'Fetch NASCAR driver standings'
  );
  const startTime = Date.now();

  try {
    sentryService.trackRacingOperation('get_driver_standings', 'nascar', {
    ...
    if (
      !process.env.NASCAR_API_KEY &&
      !process.env.SPORTS_DATA_API_KEY &&
      !process.env.NASCAR_DATA_REPO_URL
    ) {
      console.warn(
        'NASCAR data sources not configured. Please set NASCAR_DATA_REPO_URL, NASCAR_API_KEY, or SPORTS_DATA_API_KEY.'
      );
    }
...
export const getTeamStandings = async (): Promise<NascarTeam[]> => {
  const transaction = sentryService.startTransaction(
    'nascar-get-team-standings',
    'query',
    'Fetch NASCAR team standings'
  );
  const startTime = Date.now();

  try {
    sentryService.trackRacingOperation('get_team_standings', 'nascar', {
      season: new Date().getFullYear(),
    });
...
export const getRacePrediction = async (
  ...
  const transaction = sentryService.startTransaction(
    'nascar-get-race-prediction',
    'query',
    'Fetch NASCAR race prediction'
  );
  const startTime = Date.now();
