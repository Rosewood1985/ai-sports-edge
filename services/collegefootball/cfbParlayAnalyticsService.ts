...
export class CFBParlayAnalyticsService {
  ...
  async analyzeCFBParlay(parlayData: CFBParlayData): Promise<CFBParlayPrediction> {
    ...
    try {
      ...
      const gameDataPromises = parlayData.legs.map(leg => this.getGameData(leg.gameId));
      ...
      const individualProbabilities = await Promise.all(
        parlayData.legs.map((leg, index) => this.analyzeIndividualLeg(leg, gamesData[index]))
      );
  ...
  private analyzeSpread(
    ...
    const pointDifferential =
      ...
      homeStats.pointsAllowedPerGame -
    ...
    if (gameData.rivalry) {
      probability *= 0.9; // Rivalry games are unpredictable
  ...
  private analyzeTotal(
    ...
    const projectedTotal =
      (homeStats.pointsPerGame +
        awayStats.pointsAllowedPerGame +
  ...
  private async analyzePlayerProp(
    ...
    if (
      gameData.weather &&
      (leg.selection.includes('passing') || leg.selection.includes('receiving'))
    ) {
  ...
  private analyzeTeamProp(
    ...
    if (leg.selection.includes('first quarter') || leg.selection.includes('1Q')) {
      ...
      const rushingYards = leg.selection.includes(gameData.homeTeam.name)
        ? homeStats.rushingYardsPerGame
        : awayStats.rushingYardsPerGame;
  ...
  private async identifyCorrelations(
    ...
    for (let i = 0; i < legs.length; i++) {
      for (let j = i + 1; j < legs.length; j++) {
        ...
        const correlation = await this.analyzeGameCorrelation(legs[i], legs[j], game1, game2);
  ...
  private async analyzeGameCorrelation(
    ...
    const daysDifference =
      Math.abs(game1Date.getTime() - game2Date.getTime()) / (1000 * 60 * 60 * 24);
  ...
  private determineConferenceRelation(
    ...
    const isRivalry = rivalConferences.some(
      rivalry =>
  ...
  private isTrapGame(gameData: CollegeFootballGame): boolean {
    ...
    return (
      gameData.gameType === 'non_conference' &&
  ...
  private hasWeatherRisk(weather: any): boolean {
    return weather.windSpeed > 20 || weather.precipitation > 0.3 || weather.temperature < 25;
  }
  ...
  private async generateRecommendations(
    ...
    const negativeEVLegs = individualProbabilities.filter((prob, index) => {
  ...
  private performSituationalAnalysis(
    ...
    legs.forEach((leg, index) => {
      ...
      if (gameData.weather && this.hasWeatherRisk(gameData.weather)) {
        weatherConcerns.push(
          `${gameData.homeTeam.name} vs ${gameData.awayTeam.name}: Weather risk`
        );
      }
    });
  ...
  private calculateLegConfidence(
    ...
    const avgGamesPlayed =
      (homeStats.wins + homeStats.losses + awayStats.wins + awayStats.losses) / 2;
  ...
  private async storeParlayAnalysis(
    parlayId: string,
    prediction: CFBParlayPrediction
  ): Promise<void> {
    await this.db
      .collection('cfb_parlay_analysis')
      .doc(parlayId)
      .set({
        ...prediction,
