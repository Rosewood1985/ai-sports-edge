...
class OfflineQueueService {
   ...
   */
  async addToQueue<T extends QueuedAction>(action: Omit<T, 'id' | 'createdAt' | 'status' | 'retryCount'>): Promise<T> {
    try {
   ...
   */
  registerProcessor<T extends QueuedAction>(type: QueuedActionType, processor: ActionProcessor<T>): void {
   ...
   */
  registerConflictHandler<T extends QueuedAction>(entityType: string, handler: ConflictHandler<T>): void {
  ...
  async processQueue(): Promise<void> {
    ...
    try {
      ...
      const pendingActions = this.queue
        .filter(action => action.status === QueuedActionStatus.PENDING || action.status === QueuedActionStatus.RETRYING)
      ...
      for (const action of pendingActions) {
        ...
        if (!processor) {
          ...
          await this.updateActionStatus(action.id, QueuedActionStatus.FAILED, 'No processor registered');
          continue;
        }
        ...
        try {
          ...
          if (success) {
            ...
            if (updatedAction) {
              ...
              if (updatedAction.retryCount >= updatedAction.maxRetries) {
                await this.updateActionStatus(action.id, QueuedActionStatus.FAILED, 'Max retries reached');
              } else {
          ...
          if (updatedAction) {
            ...
            // Check if max retries reached
            if (updatedAction.retryCount >= updatedAction.maxRetries) {
              await this.updateActionStatus(action.id, QueuedActionStatus.FAILED, (error as Error).message);
            } else {
              await this.updateActionStatus(action.id, QueuedActionStatus.RETRYING, (error as Error).message);
            }
          }
        }
      }
   ...
   */
  private async updateActionStatus(actionId: string, status: QueuedActionStatus, error?: string): Promise<void> {
    // Find action
  ...
  private registerDefaultProcessors(): void {
    ...
    this.registerProcessor<DataSyncAction>(QueuedActionType.DATA_SYNC, async (action) => {
      try {
        ...
        console.error(`Error processing data sync for ${action.entityType}:${action.entityId}:`, error);
        throw error;
      }
    });
