...
export const hasPremiumAccess = async (userId: string): Promise<boolean> => {
  try {
    ...
    if (ENV_CONFIG.IS_DEV || !isFirebaseInitialized(firebaseService.firestore.instance)) {
    ...
    try {
      const purchasesSnapshot = await firebaseService.firestore.getCollection(
        ...
        [
          ...
          firebaseService.firestore.where('expiresAt', '>', now),
          firebaseService.firestore.limit(1)
        ]
      );
      
      return purchasesSnapshot.length > 0;
    } catch (dbError) {
...
export const getSubscriptionStatus = async (userId: string): Promise<SubscriptionStatus | null> => {
  try {
    ...
    if (!userDoc || !(userDoc as any).subscriptionId) {
      return null;
    }
    
    const subscriptionId = (userDoc as any).subscriptionId;
    ...
    const data = subscriptionDoc as any;
...
export const subscribeToPlan = async (
  ...
  try {
    ...
    return (result as any).status === 'active' || (result as any).status === 'trialing';
  } catch (error) {
...
export const cancelSubscription = async (
  ...
  try {
    ...
    const result = await firebaseService.functions.callFunction('cancelSubscription', {
      userId,
      subscriptionId,
      immediate
    });
    
    return (result as any).canceled === true;
  } catch (error) {
...
export const getUserPaymentMethods = async (userId: string): Promise<PaymentMethod[]> => {
  try {
    const paymentMethodsCollection = await firebaseService.firestore.getCollection(
      `users/${userId}/paymentMethods`
    );
    
    if (!paymentMethodsCollection || paymentMethodsCollection.length === 0) {
      return [];
    }
    
    return paymentMethodsCollection.map((doc: any) => {
      return {
        id: doc.id,
        brand: doc.brand || 'unknown',
        last4: doc.last4 || '0000',
        expiryMonth: doc.expiryMonth || 0,
        expiryYear: doc.expiryYear || 0,
...
export const purchaseOneTimeProduct = async (
  ...
  try {
    ...
    const result = await firebaseService.functions.callFunction('createOneTimePayment', {
      userId,
      paymentMethodId,
      productId,
    ...
    return (result as any).status === 'succeeded';
  } catch (error) {
...
export const hasGamePredictionAccess = async (
  ...
  try {
    ...
    const purchasesCollection = await firebaseService.firestore.getCollection(
      `users/${userId}/purchases`,
      [
        firebaseService.firestore.where('productId', '==', 'single-prediction'),
        firebaseService.firestore.where('gameId', '==', gameId),
        firebaseService.firestore.where('status', '==', 'succeeded'),
        firebaseService.firestore.limit(1)
      ]
    );
    
    return purchasesCollection.length > 0;
  } catch (error) {
...
export const updatePaymentMethod = async (
  ...
  try {
    ...
    const result = await firebaseService.functions.callFunction('updatePaymentMethod', {
      userId,
      paymentMethodId
    });
    
    return (result as any).updated === true;
  } catch (error) {
...
export const updateSubscription = async (
  ...
  try {
    const result = await firebaseService.functions.callFunction('updateSubscription', {
      userId,
      subscriptionId,
      newPriceId,
      immediate
...
export const pauseSubscription = async (
  ...
  try {
    const result = await firebaseService.functions.callFunction('pauseSubscription', {
      userId,
      subscriptionId,
      pauseDuration
    });
...
export const resumeSubscription = async (
  ...
  try {
    const result = await firebaseService.functions.callFunction('resumeSubscription', {
      userId,
      subscriptionId
    });
    return result;
...
export const giftSubscription = async (
  ...
  try {
    const result = await firebaseService.functions.callFunction('giftSubscription', {
      userId,
      recipientEmail,
      priceId,
      paymentMethodId,
...
export const toggleAutoResubscribe = async (
  ...
  try {
    const result = await firebaseService.functions.callFunction('toggleAutoResubscribe', {
      userId,
      subscriptionId,
      enabled
    });
...
export const generateReferralCode = async (
  ...
  try {
    const result = await firebaseService.functions.callFunction('generateReferralCode', {
      userId
    });
...
export const applyReferralCode = async (
  ...
  try {
    const result = await firebaseService.functions.callFunction('applyReferralCode', {
      newUserId: userId,
      referralCode
    });
    ...
    await trackEvent('referral_code_applied', {
      referralCode,
      referrerId: (result as any).referrerId
    });
...
export const redeemGiftSubscription = async (
  ...
  try {
    const result = await firebaseService.functions.callFunction('redeemGiftSubscription', {
      giftCode
    });
...
export const hasUsedFreeDailyPick = async (userId: string): Promise<boolean> => {
  try {
    ...
    if ((userDoc as any).lastFreeDailyPick) {
      const lastPickDate = (userDoc as any).lastFreeDailyPick.toDate();
