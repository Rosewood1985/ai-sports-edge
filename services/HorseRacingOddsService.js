...
class HorseRacingOddsService {
  ...
  async getRaceOdds(raceId, includeWeather = false) {
    try {
      ...
      const outcomes = race.bookmakers.flatMap(bookmaker =>
        bookmaker.markets.filter(market => market.key === 'win').flatMap(market => market.outcomes)
      );
        ...
        filteredRaces.map(async race => {
          ...
          if (includeWeather) {
            ...
            const {
              ...
              weatherAdjustment,
              weatherData,
            } = await weatherAdjustmentService.getWeatherAdjustedOdds(
              race.id,
              'HORSE_RACING',
              outcomes
            );
  ...
  async getWeatherImpact() {
    try {
      ...
      const racesWithWeatherImpact = await Promise.all(
