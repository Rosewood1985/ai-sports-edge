...
PredictionSchema.methods.evaluatePrediction = function (gameResult) {
  ...
  if (
    this.predictions.score.homeScore.prediction !== undefined &&
    this.predictions.score.awayScore.prediction !== undefined &&
    gameResult.homeScore !== undefined &&
    gameResult.awayScore !== undefined
  ) {
    const homeScoreDiff = Math.abs(
      this.predictions.score.homeScore.prediction - gameResult.homeScore
    );
    const awayScoreDiff = Math.abs(
      this.predictions.score.awayScore.prediction - gameResult.awayScore
    );

    // Calculate percentage accuracy (100% - percentage error)
    const homeMaxScore = Math.max(
      this.predictions.score.homeScore.prediction,
      gameResult.homeScore
    );
    const awayMaxScore = Math.max(
      this.predictions.score.awayScore.prediction,
