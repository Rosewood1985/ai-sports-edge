  ...
  const history = await model.fit(xs, ys, {
    ...
    callbacks: {
      onEpochEnd: (epoch, logs) => {
        if (epoch % 10 === 0) {
          console.log(
            `Epoch ${epoch}: loss = ${logs.loss.toFixed(4)}, accuracy = ${logs.acc.toFixed(4)}, val_loss = ${logs.val_loss.toFixed(4)}, val_accuracy = ${logs.val_acc.toFixed(4)}`
          );
        }
      },
    },
...
async function trainModels(sport, predictionType) {
  ...
  const tfEvaluation = await evaluateModel(tfResult.model, 'tf', testFeatures, testLabels);

  // Save TensorFlow.js model
  await saveTensorFlowModel(tfResult.model, sport, predictionType, {
    ...tfResult.metrics,
    ...tfEvaluation,
  });

  // Train Random Forest model
  const rfResult = trainRandomForestModel(trainFeatures, trainLabels, valFeatures, valLabels);

  // Evaluate Random Forest model
  const rfEvaluation = await evaluateModel(rfResult.model, 'rf', testFeatures, testLabels);
