...
router.get('/status', authenticate, adminAuth, async (req, res) => {
  try {
    ...
    const overUnderMetrics =
      await SelfLearningService.performanceEvaluator.evaluateOverUnderModel();
    const moneylineMetrics =
      await SelfLearningService.performanceEvaluator.evaluateMoneylineModel();
...
router.get('/performance/:sport', authenticate, adminAuth, async (req, res) => {
  try {
    ...
    const metrics =
      await SelfLearningService.performanceEvaluator.evaluatePerformanceBySport(sport);
    ...
    res
      .status(500)
      .json({ error: `Failed to get model performance metrics for ${req.params.sport}` });
  }
});
