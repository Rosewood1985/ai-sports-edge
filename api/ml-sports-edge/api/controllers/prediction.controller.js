...
function loadRandomForestModel(sport, predictionType) {
  ...
  try {
    ...
    console.error(`Error loading Random Forest model metadata for ${sport} ${predictionType}:`, error);
    throw error;
  }
}
    ...
    const enhancedPrediction = {
      ...
      analysis: {
        ...
        modelInsights: {
          ...
          homeTeamRecentForm: 0.70,
          awayTeamRecentForm: 0.60,
          h2hHomeWinRate: 0.60
        }
      }
    };
...
async function getPredictionsBySport(req, res) {
  try {
    ...
    // Validate sport
    const validSports = ['NBA', 'WNBA', 'MLB', 'NHL', 'NCAA_MENS', 'NCAA_WOMENS', 'FORMULA1', 'UFC'];
...
async function getTrendingPredictions(req, res) {
  try {
    ...
    trendingPredictions.sort((a, b) => {
      ...
      const bConfidence = b.predictions?.spread?.confidence || 
                         b.predictions?.winner?.confidence || 
                         b.predictions?.moneyline?.confidence || 0;
