...
function normalizeNBAData(sportData) {
  ...
  games.forEach(game => {
    ...
    if (sources.espn && sources.espn.statistics) {
      const homeTeamStats = sources.espn.statistics?.teams?.find(
        team => team.id === game.homeTeam.id
      );

      const awayTeamStats = sources.espn.statistics?.teams?.find(
    ...
    if (sources.espn && sources.espn.standings) {
      const homeTeamStanding = sources.espn.standings?.teams?.find(
        team => team.id === game.homeTeam.id
      );

      const awayTeamStanding = sources.espn.standings?.teams?.find(
        team => team.id === game.awayTeam.id
      );
    ...
    if (sources.espn && sources.espn.gameDetails) {
      const gameDetail = sources.espn.gameDetails.find(detail => detail.id === game.id);
  ...
  games.forEach(game => {
    ...
    if (sources.nhlStats) {
      const nhlGame = sources.nhlStats.dates?.[0]?.games?.find(g => {
        const homeMatch =
          g.teams.home.team.name.includes(game.homeTeam.name) ||
          game.homeTeam.name.includes(g.teams.home.team.name);
        const awayMatch =
          g.teams.away.team.name.includes(game.awayTeam.name) ||
  ...
  games.forEach(game => {
    ...
    if (sources.ncaaSchedule) {
      const ncaaGame = sources.ncaaSchedule.games?.find(g => {
        const homeMatch =
          g.home.name.includes(game.homeTeam.name) || game.homeTeam.name.includes(g.home.name);
        const awayMatch =
          g.away.name.includes(game.awayTeam.name) || game.awayTeam.name.includes(g.away.name);
        return homeMatch && awayMatch;
      });
  ...
  if (sources.espn && sources.espn.events) {
    sources.espn.events.forEach(event => {
      ...
      const location = event.circuit?.address?.city
        ? `${event.circuit.address.city}, ${event.circuit.address.country}`
        : 'TBD';
  ...
  if (sources.odds && Array.isArray(sources.odds)) {
    sources.odds.forEach(oddsItem => {
      ...
      const race = races.find(
        r => r.raceName.includes(oddsItem.description) || oddsItem.description.includes(r.raceName)
      );
      ...
      if (race) {
        ...
        const bookmaker =
          oddsItem.bookmakers.find(
            b => b.key === 'fanduel' || b.title.toLowerCase().includes('fanduel')
  ...
  if (sources.odds && Array.isArray(sources.odds)) {
    sources.odds.forEach(oddsItem => {
      ...
      const fight = fights.find(f => {
        const fighter1Match =
          f.fighter1.name.includes(oddsItem.home_team) ||
          oddsItem.home_team.includes(f.fighter1.name);
        const fighter2Match =
          f.fighter2.name.includes(oddsItem.away_team) ||
      ...
      if (fight) {
        ...
        if (bookmaker) {
          ...
          const h2hMarket = bookmaker.markets.find(m => m.key === 'h2h');
          if (h2hMarket && h2hMarket.outcomes) {
            const fighter1Odds = h2hMarket.outcomes.find(
              o => o.name.includes(fight.fighter1.name) || fight.fighter1.name.includes(o.name)
            );
            const fighter2Odds = h2hMarket.outcomes.find(
              o => o.name.includes(fight.fighter2.name) || fight.fighter2.name.includes(o.name)
            );
