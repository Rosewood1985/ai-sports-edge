...
class SelfLearningService {
    ...
    try {
      ...
      for (const game of games) {
        ...
        if (results) {
          ...
          await game.save();
          logger.info(
            `Updated results for game ${game._id}: ${game.homeTeam.name} ${results.homeScore} - ${results.awayScore} ${game.awayTeam.name}`
          );
  ...
  checkRetrainingNeeded(metrics) {
    ...
    if (metrics.spread.accuracy < this.config.thresholds.accuracy) {
      logger.info(
        `Spread model accuracy (${metrics.spread.accuracy}) below threshold (${this.config.thresholds.accuracy})`
      );
      return true;
    }
    ...
    if (metrics.overUnder.accuracy < this.config.thresholds.accuracy) {
      logger.info(
        `Over/Under model accuracy (${metrics.overUnder.accuracy}) below threshold (${this.config.thresholds.accuracy})`
      );
      return true;
    }
    ...
    if (metrics.moneyline.accuracy < this.config.thresholds.accuracy) {
      logger.info(
        `Moneyline model accuracy (${metrics.moneyline.accuracy}) below threshold (${this.config.thresholds.accuracy})`
      );
      return true;
    }
    ...
    if (metrics.score.mse > this.config.thresholds.errorRate) {
      logger.info(
        `Score model MSE (${metrics.score.mse}) above threshold (${this.config.thresholds.errorRate})`
      );
      return true;
    }
    ...
    if (metrics.spread.calibration && metrics.spread.calibration.averageCalibrationError > 0.1) {
      logger.info(
        `Spread model calibration error (${metrics.spread.calibration.averageCalibrationError}) above threshold (0.1)`
      );
      return true;
    }
