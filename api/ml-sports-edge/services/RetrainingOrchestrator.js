...
class RetrainingOrchestrator {
  ...
  async prepareTrainingData() {
    try {
      ...
      logger.info(
        `Split data into ${trainingSet.length} training, ${validationSet.length} validation, and ${testSet.length} test samples`
      );
  ...
  async analyzePredictionErrors(predictions) {
    try {
      ...
      logger.info(
        `Analyzing ${correctPredictions.length} correct and ${incorrectPredictions.length} incorrect predictions`
      );

      // Analyze feature importance in correct vs. incorrect predictions
      const featureAnalysis = this.analyzeFeatureImportance(
        correctPredictions,
  ...
  analyzeConfidenceCalibration(predictions) {
    ...
    Object.keys(confidenceBins).forEach(bin => {
      ...
      confidenceBins[bin].calibrationError = Math.abs(
        confidenceBins[bin].accuracy - parseFloat(bin)
      );
    });
  ...
  analyzeSportPatterns(predictions) {
    ...
    Object.keys(sportGroups).forEach(sport => {
      ...
      group.accuracy = {
        spread:
          group.correct.spread + group.incorrect.spread > 0
            ? group.correct.spread / (group.correct.spread + group.incorrect.spread)
            : 0,
        overUnder:
          group.correct.overUnder + group.incorrect.overUnder > 0
            ? group.correct.overUnder / (group.correct.overUnder + group.incorrect.overUnder)
            : 0,
        moneyline:
          group.correct.moneyline + group.incorrect.moneyline > 0
  ...
  async analyzeTeamPatterns(predictions) {
    ...
    Object.keys(teamGroups).forEach(team => {
      ...
      group.overall = {
        ...
        accuracy:
          group.home.total + group.away.total > 0
  ...
  async retrainModels(trainingData) {
    try {
      ...
      const validationResults = await this.evaluateModels(
        ModelService.models,
        enhancedTrainingData.validationSet
      );
