...
class PerformanceEvaluator {
  ...
  async evaluateSpreadModel() {
    try {
      ...
      const correctPredictions = predictions.filter(
        p => p.result.spreadResult === 'correct'
      ).length;
  ...
  async evaluateOverUnderModel() {
    try {
      ...
      const totalPredictions = predictions.length;
      const correctPredictions = predictions.filter(
        p => p.result.overUnderResult === 'correct'
      ).length;
      ...
      const overCorrect = overPredictions.filter(
        p => p.result.overUnderResult === 'correct'
      ).length;
      ...
      const incorrectPredictions = predictions.filter(
        p => p.result.overUnderResult === 'incorrect'
      );
      const errorPatterns = this.analyzeErrorPatterns(incorrectPredictions);
  ...
  async evaluateMoneylineModel() {
    try {
      ...
      const totalPredictions = predictions.length;
      const correctPredictions = predictions.filter(
        p => p.result.moneylineResult === 'correct'
      ).length;
      ...
      const homeCorrect = homePredictions.filter(
        p => p.result.moneylineResult === 'correct'
      ).length;
      ...
      // Analyze error patterns
      const incorrectPredictions = predictions.filter(
        p => p.result.moneylineResult === 'incorrect'
      );
      const errorPatterns = this.analyzeErrorPatterns(incorrectPredictions);
  ...
  calculateROI(predictions, type) {
    ...
    predictions.forEach(prediction => {
      ...
      if (prediction.result[`${type}Result`] === 'correct') {
        const odds =
          prediction.predictions[type].pick === 'home'
  ...
  calculateMetrics(predictions, type) {
    ...
    const filteredPredictions = predictions.filter(
      p => p.result[`${type}Result`] === 'correct' || p.result[`${type}Result`] === 'incorrect'
    );

    // Calculate accuracy
    const totalPredictions = filteredPredictions.length;
    const correctPredictions = filteredPredictions.filter(
      p => p.result[`${type}Result`] === 'correct'
    ).length;
    const accuracy = totalPredictions > 0 ? correctPredictions / totalPredictions : 0;
