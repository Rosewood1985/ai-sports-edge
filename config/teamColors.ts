...
export const getLeagueTeams = (league: string): (TeamColor & { id: string })[] => {
  const leagueData = teamColorsData[league as keyof TeamColorsDatabase];
...
export const getAllTeams = (): (TeamColor & { id: string; league: string })[] => {
  const teams: (TeamColor & { id: string; league: string })[] = [];
