      ...
      if (isNaN(num)) {
        switch (char.toLowerCase()) {
          case 'f':
            return 99; // Fell
          case 'u':
            return 98; // Unseated rider
          case 'p':
            return 97; // Pulled up
          case 'r':
            return 96; // Refused
          case 'b':
            return 95; // Brought down
          default:
            return 0;
        }
      }
...
export const calculateNascarPoints = (
  position: number,
  lapsLed: number = 0,
...
export const determineNascarTrackType = (
  trackName: string
...
export const createRacingError = (
  message: string,
  type: 'validation' | 'api' | 'cache' | 'processing' = 'processing'
): Error => {
...
export const measureRacingApiPerformance = <T>(
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      console.error(
        `Racing API operation "${operationName}" failed after ${duration.toFixed(2)}ms:`,
        error
      );
