...
const PersonalizationSettings: React.FC<PersonalizationSettingsProps> = ({ onClose }) => {
  const { preferences, isLoading, setDefaultSport, setDefaultSportsbook, resetPreferences } =
    usePersonalization();
  const { colors, isDark } = useTheme();
  const { t } = useI18n();

  // Local state for UI
  const [activeTab, setActiveTab] = useState<'general' | 'sportsbooks' | 'notifications'>(
    'general'
  );
  ...
  const handleSportSelection = async (sport: string) => {
    ...
    Alert.alert(
      ...
      t('personalization.alerts.defaultSportUpdatedMessage', {
        sport: AVAILABLE_SPORTS.find(s => s.key === sport)?.name || sport,
      }),
  ...
  const renderGeneralTab = () => (
    <View style={styles.tabContent}>
      <Text style={[styles.sectionTitle, { color: colors.text }]}>
        {t('personalization.general.defaultSport')}
      </Text>
      ...
      <ScrollView style={styles.optionsList}>
        {AVAILABLE_SPORTS.map(sport => (
          ...
          >
            <Text style={[styles.optionText, { color: colors.text }]}>{sport.name}</Text>
            {preferences.defaultSport === sport.key && (
  ...
  const renderSportsbooksTab = () => (
    <View style={styles.tabContent}>
      <Text style={[styles.sectionTitle, { color: colors.text }]}>
        {t('personalization.sportsbooks.defaultSportsbook')}
      </Text>
      <Text style={[styles.sectionDescription, { color: colors.text }]}>
      ...
      <View style={styles.optionsList}>
        ...
        <TouchableOpacity
          style={[
            ...
            {
              backgroundColor:
                preferences.defaultSportsbook === null ? colors.primary + '20' : colors.background,
            },
          ]}
          onPress={() => handleSportsbookSelection(null)}
        >
          <Text style={[styles.optionText, { color: colors.text }]}>
            {t('personalization.sportsbooks.noPreference')}
          </Text>
    ...
    <View style={styles.tabContent}>
      ...
      <View style={styles.optionsList}>
        <View style={styles.switchOption}>
          <Text style={[styles.optionText, { color: colors.text }]}>
            {t('personalization.notifications.oddsMovements')}
          </Text>
          <Switch
            ...
            thumbColor={
              preferences.notificationPreferences?.oddsMovements ? colors.primary : '#f4f3f4'
            }
          />
        </View>
        ...
        <View style={styles.switchOption}>
          <Text style={[styles.optionText, { color: colors.text }]}>
            {t('personalization.notifications.gameStart')}
          </Text>
          <Switch
            ...
            trackColor={{ false: '#767577', true: colors.primary + '80' }}
            thumbColor={preferences.notificationPreferences?.gameStart ? colors.primary : '#f4f3f4'}
          />
        </View>

        <View style={styles.switchOption}>
          <Text style={[styles.optionText, { color: colors.text }]}>
            {t('personalization.notifications.gameEnd')}
          </Text>
          <Switch
            ...
            trackColor={{ false: '#767577', true: colors.primary + '80' }}
            thumbColor={preferences.notificationPreferences?.gameEnd ? colors.primary : '#f4f3f4'}
          />
        </View>

        <View style={styles.switchOption}>
          <Text style={[styles.optionText, { color: colors.text }]}>
            {t('personalization.notifications.specialOffers')}
          </Text>
          <Switch
            ...
            trackColor={{ false: '#767577', true: colors.primary + '80' }}
            thumbColor={
              preferences.notificationPreferences?.specialOffers ? colors.primary : '#f4f3f4'
            }
          />
        </View>
      </View>
