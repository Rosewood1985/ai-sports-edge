...
import { createOptimizedGlowStyle } from './OptimizedOddsAnimations'; // iPhone SE or similar small device
 ...
 */
const OddsComparisonComponent = forwardRef<
  OddsComparisonComponentRef,
  OddsComparisonComponentProps
>(({ isPremium = false }, ref) => {
  // Generate a unique component ID for cleanup registration
  const componentId = useRef(
    `odds_comparison_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`
  ).current;
  ...
  const [dataSource, setDataSource] = useState<'api' | 'cache' | 'stale' | 'fallback' | 'error'>(
    'api'
  );
  ...
  const processOddsData = memoizeWithTTL(
    async (data: Game[]) => {
      ...
      const gamesWithBothBookmakers = data.filter(
        (game: Game) =>
      ...
      const dkBookmaker = selectedGame.bookmakers.find(
        (book: Bookmaker) => book.key === 'draftkings'
      );
  ...
  const generateMockData = (): Game[] => {
    ...
    const sportInfo =
      AVAILABLE_SPORTS.find(sport => sport.key === selectedSport) || AVAILABLE_SPORTS[0];
  ...
  const renderErrorState = () => (
    <View style={styles.errorContainer}>
      ...
      <Text style={[styles.errorTitle, { color: colors.text }]}>Error Loading Odds</Text>
  ...
  const handlePurchaseOdds = async () => {
    try {
      ...
      if (!currentUserId) {
        Alert.alert('Sign In Required', 'Please sign in to purchase odds access.', [
          { text: 'OK' },
        ]);
        return;
      }
          ...
          },
          {
            ...
            onPress: async () => {
              ...
              setTimeout(async () => {
                ...
                if (currentUserId) {
                  bettingAffiliateService.trackConversion(
                    'odds_purchase',
                    price / 100,
                    currentUserId
                  );
  ...
  const handleSportsbookClick = async (sportsbook: 'draftkings' | 'fanduel') => {
    ...
    try {
      ...
      setLoading(false);
      Alert.alert('Error', 'Could not open sportsbook. Please try again.', [{ text: 'OK' }]);
    }
  };
        ...
        <>
          <View style={styles.headerContainer}>
            ...
            <View style={styles.lastUpdatedContainer}>
              ...
              <TouchableOpacity style={styles.refreshButton} onPress={fetchOdds} disabled={loading}>
                ...
                <View style={styles.filterRow}>
                  ...
                  <View style={styles.checkboxContainer}>
                    <TouchableOpacity
                      ...
                      accessibilityHint={
                        filterOptions.showDraftKings
                          ? 'Hide DraftKings odds'
                          : 'Show DraftKings odds'
                      }
                      accessibilityActions={[{ name: 'activate' }]}
                      onAccessibilityAction={({ nativeEvent }) => {
                        if (nativeEvent.actionName === 'activate') {
                    ...
                    <TouchableOpacity
                      ...
                      accessibilityHint={
                        filterOptions.showFanDuel ? 'Hide FanDuel odds' : 'Show FanDuel odds'
                      }
                      accessibilityActions={[{ name: 'activate' }]}
                      onAccessibilityAction={({ nativeEvent }) => {
                        if (nativeEvent.actionName === 'activate') {
