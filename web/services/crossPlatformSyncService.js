    ...
    try {
      ...
      if (cloudData) {
        ...
        if (cloudData.purchasedOdds && cloudData.purchasedOdds.length > 0) {
          ...
          localStorage.setItem('purchased_odds', JSON.stringify(this.purchasedOdds));
        ...
        if (cloudData.userPreferences) {
          ...
          if (
            ...
            localStorage.setItem('primary_team', this.userPreferences.primaryTeam);
            ...
            localStorage.setItem('affiliate_code', this.userPreferences.affiliateCode);
          }
        }
      }
  ...
  async updateUserPreferences(preferences) {
    try {
      ...
      if (preferences.favoriteTeams) {
        localStorage.setItem('favorite_teams', JSON.stringify(preferences.favoriteTeams));
      }

      if (preferences.primaryTeam) {
        localStorage.setItem('primary_team', preferences.primaryTeam);
      }

      if (preferences.buttonSettings) {
        localStorage.setItem('button_settings', JSON.stringify(preferences.buttonSettings));
      }

      if (preferences.affiliateEnabled !== undefined) {
        localStorage.setItem('affiliate_enabled', String(preferences.affiliateEnabled));
      }

      if (preferences.affiliateCode) {
        localStorage.setItem('affiliate_code', preferences.affiliateCode);
      }
