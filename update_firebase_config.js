...
if (config.hosting.rewrites) {
  config.hosting.rewrites.forEach(rewrite => {
    if (rewrite.destination && rewrite.destination.includes('api.ai-sports-edge.com')) {
      rewrite.destination = rewrite.destination.replace('api.ai-sports-edge.com', 'api.aisportsedge.app');
    }
    if (rewrite.destination && rewrite.destination.includes('admin.ai-sports-edge.com')) {
      rewrite.destination = rewrite.destination.replace('admin.ai-sports-edge.com', 'admin.aisportsedge.app');
    }
  });
}
...
if (config.hosting.redirects) {
  const newRedirects = config.hosting.redirects.filter(redirect => 
    !redirect.source.includes('ai-sports-edge.com')
  );
